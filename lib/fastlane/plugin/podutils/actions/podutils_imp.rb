# represents the pod version in the for major.minor.build
# for example '0.1.2'
module Fastlane
 	module Pod
		class PodVersion
		...
		class Podutils
			...
			def initialize(params = {})
				...
				@podspec = params.fetch(:podspec, nil)
				@podrepo = params.fetch(:podrepo, nil)
				@verbose = params.fetch(:verbose, false)
				@bumpMethod = params.fetch(:bumpMethod, 0)
				@skipLibLint = params.fetch(:skipLibLint, false)
				@noClean = params.fetch(:noClean, false)
				@libLintOnly = params.fetch(:libLintOnly, false)

				if (@podspec == nil) 
					raise StandardError.new("no podspec specified")
				end

				if (@podrepo == nil && @libLintOnly != true)
		...
		A simple utility for fastlane to ease releasing pods

		usage: 	to release or verify:
				podutils = Podutils.new(parameters)
			   	podutils.release_pod()

			   	to list these options:
			   	Podutils.list_options() 

		where parameters are a combination of the following:

		:podspec - the podspec name. for example: 'ybs-core.podspec'.
					mandatory argument, omitting will raise an error.

		:podrepo - the pod repo name. for example: 'ybs-core'
					this can be ommitted if :libLintOnly is set, otherwise an error is raised.

		:verbose - set to true for verbose logging.  
					optional argument, default is false.

		:bumpMethod - whether to bump major, minor or build.  
					optional argument, default is BUMP_BUILD
					options are: BUMP_MAJOR, BUMP_MINOR or BUMP_BUILD.
					attempting to set any other value will raise an error.

		:skipLibLint - set 'true' to skip 'pod lib lint'.  
					optional argument, default is false

		:noClean - set 'true' to not clean up or rollback on error.  
					optional argument, default is false

		:libLintOnly - set 'true' to only perform 'pod lib lint'.  
					optional argument, default is false
					if both :libLintOnly and :skipLibLint are set, the 
				STR
			end
			...
			def run()
				...
				if (!File.exists?(@podspec))
					raise StandardError.new("no #{@podspec} found")
				end
